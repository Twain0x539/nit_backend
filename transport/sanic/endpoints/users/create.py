...
class CreateUserEndpoint(BaseEndpoint):
    ...
    async def method_post(self, request: Request, body: dict, session: DBSession, *args, **kwargs) -> BaseHTTPResponse:
        ...
        try:
            ...
            return await self.make_responce_json(status=409, message='User already exists')
        except (DBIntegrityException, DBDataException) as e:
            return await self.make_responce_json(status=500, message=str(e))

        response_model = ResponseUserDto(db_user)

        return await self.make_responce_json(status=201, body=response_model.dump())